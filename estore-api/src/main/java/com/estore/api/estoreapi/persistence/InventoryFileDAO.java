...
/**
 * updates product information in inventory
 */
@Component
public class InventoryFileDAO implements InventoryDAO {
    ...
    private static int nextId;  // The next Id to assign to a new product in the inventory
    ...
    public InventoryFileDAO(@Value("${products.file}") String filename, ObjectMapper objectMapper) throws IOException {
        ...
        load(); // load products from the file
    }
     ...
     */
    public Product[] findProducts(String containsText, Integer containsPrice) throws IOException {
        synchronized(products) {
    ...
    public Product createProduct(Product product) throws IOException {
        synchronized(products) {
            if (getProductsArray(product.getName(), null).length == 0){
                if(product.getQuantity() > 0) {
                    Product newProduct = new Product (product.getName(), product.getPrice(), product.getQuantity(), nextId());
